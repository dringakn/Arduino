/*
Name:		MovingAverageFilterDynamic.h
Created:	12/12/2017 10:05:56 AM
Author:		Dr. -Ing. Ahmad Kamal Nasir (dringakn@gmail.com, http://web.lums.edu.pk/~akn/)
License:	This Library is licensed under a GPLv3 License
*/

#ifndef _MOVINGAVERAGEFILTERDYNAMIC_h
#define _MOVINGAVERAGEFILTERDYNAMIC_h
...
template<typename T> 
class MovingAverageFilterDynamic
{
 protected:
 ...
 public:
	void init(int);
	MovingAverageFilterDynamic(int);
	~MovingAverageFilterDynamic();
...
template<typename T>
void MovingAverageFilterDynamic<T>::init(int nSamples)
{
	if (nSamples != winSize) {
...
template<typename T>
MovingAverageFilterDynamic<T>::MovingAverageFilterDynamic(int nSamples = 10)
{
	winSize = nSamples;
...
template<typename T>
MovingAverageFilterDynamic<T>::~MovingAverageFilterDynamic()
{
	free(array);
}

template<typename T>
unsigned int MovingAverageFilterDynamic<T>::getWinSize()
{
	return winSize;
}

template<typename T>
T MovingAverageFilterDynamic<T>::filter(T data)
{
...
template<typename T>
void MovingAverageFilterDynamic<T>::show()
{
	for (i = 0; i < winSize; i++) {
