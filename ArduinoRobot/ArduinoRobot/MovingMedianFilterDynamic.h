/*
Name:		MovingMedianFilterDynamic.h
...
template<typename T> 
class MovingMedianFilterDynamic
{
 protected:
 ...
 public:
	 MovingMedianFilterDynamic(int);
	 ~MovingMedianFilterDynamic();
...
template<typename T>
MovingMedianFilterDynamic<T>::MovingMedianFilterDynamic(int nSamples = 10)
{
	winSize = nSamples;
...
template<typename T>
MovingMedianFilterDynamic<T>::~MovingMedianFilterDynamic()
{
	free(array);
	free(tempArray);
}
...
template<typename T>
unsigned int MovingMedianFilterDynamic<T>::getWinSize()
{
	return winSize;
}

template<typename T>
void MovingMedianFilterDynamic<T>::init(int nSamples)
{
	if (nSamples != winSize) {
...
template<typename T>
T MovingMedianFilterDynamic<T>::filter(T data)
{
...
template<typename T>
void MovingMedianFilterDynamic<T>::show()
{
	for (i = 0; i < winSize; i++) {
