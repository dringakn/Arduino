/*
Name:		Robot_US_Sensor_Test_Example.ino
...
Purpose:	The following program consist of two parts. Setup will initialize the robot while the loop will run indefnitely
			to repeatedly check if any one of the three US sensor detected a obstacle. If a obstacle is detected then the
...
void loop() {
	// If Obstacle is in any direction: 1 1 1
	if (robot.bUSFront || robot.bUSLeft || robot.bUSRight) {
